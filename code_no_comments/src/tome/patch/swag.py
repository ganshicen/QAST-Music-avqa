...
import torch
from src.tome.merge import bipartite_soft_matching, merge_source, merge_wavg
...
def make_block_class(block_cls):
    ...
    class ToMeBlock(block_cls):
        ...
        def forward(self, input: torch.Tensor) -> torch.Tensor:
            ...
            if r > 0:
                (merge, _) = bipartite_soft_matching(metric, r, self._tome_info['class_token'], self._tome_info['distill_token'])
...
def apply_patch(model, trace_source: bool=False, prop_attn: bool=True):
    ...
    if BlockClass is None or EncoderClass is None:
        print("Error patching model: this model isn't a SWAG transformer or the interface has been updated.")
        return
    ...
    model.r = 0
    model._tome_info = {'r': model.r, 'size': None, 'source': None, 'trace_source': trace_source, 'prop_attn': prop_attn, 'class_token': model.classifier == 'token', 'distill_token': False}
    for module in model.modules():
