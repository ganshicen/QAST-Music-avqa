...
$Names = @("without_tacl", "without_sacl")

function Get-OutputDir([string]$Name) {
    return "D:\model\qast_ehr_ablation_${Name}_seed713"
}
...
function Get-CompletedResult([string]$Name, [string]$ConfigPath, [string]$LogPath, [string]$ValidationPath, [string]$OutputDir) {
    if (-not (Test-Path -LiteralPath $LogPath) -or -not (Test-Path -LiteralPath $ValidationPath)) { return $null }
    $Log = Get-Content -LiteralPath $LogPath -Raw
    if (-not ($Log.Contains("training epoch 15") -and $Log.Contains("Epoch 15 done"))) { return $null }
...
function Save-Manifest($Results) {
    $Manifest = [ordered]@{
        ...
        loss_weights = [ordered]@{
            baseline_lambda_tacl = 0.035
            baseline_lambda_sacl = 0.035
            without_tacl_lambda_tacl = 0.0
            without_sacl_lambda_sacl = 0.0
        }
        results = @($Results)
    }
...
try {
    foreach ($Name in $Names) {
        $ConfigPath = (Resolve-Path "experiments\qast_ehr\tacl_sacl_ablations\configs\${Name}_seed713.py").Path
        ...
        $Result | Add-Member -NotePropertyName seed -NotePropertyValue 713 -Force
        $Result | Add-Member -NotePropertyName disabled_loss -NotePropertyValue $(if ($Name -eq "without_tacl") { "lambda_tacl" } else { "lambda_sacl" }) -Force
        $Result | Add-Member -NotePropertyName disabled_weight -NotePropertyValue 0.0 -Force
